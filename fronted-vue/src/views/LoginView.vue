      ...
      <form @submit.prevent="handleLogin">
        <div class="form-group mb-4">
          ...
          <div class="password-input-wrapper">
            ...
            />
            <span style="position: absolute; right: 0.5rem; display: inline-flex;" :title="(showPassword ? 'Скрыть пароль' : 'Показать пароль') + (isLoading ? ' - Действие недоступно во время загрузки' : '')">
              <button
                type="button"
                class="password-toggle-btn"
                @click="showPassword = !showPassword"
                :disabled="isLoading"
                :aria-label="(showPassword ? 'Скрыть пароль' : 'Показать пароль') + (isLoading ? ' - Действие недоступно во время загрузки' : '')"
              >
