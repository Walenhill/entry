    ...
    <div class="modal-content" role="dialog" aria-modal="true" aria-labelledby="booking-modal-title">
      <div class="modal-header">
        ...
        <span :title="isSubmitting ? 'Закрыть (Esc) - Действие недоступно во время загрузки' : 'Закрыть (Esc)'" style="display: inline-flex;">
          <button class="close-btn" @click="$emit('close')" :disabled="isSubmitting" :aria-label="isSubmitting ? 'Закрыть (Esc) - Действие недоступно во время загрузки' : 'Закрыть (Esc)'" aria-keyshortcuts="Escape"><span aria-hidden="true">×</span></button>
        </span>
      </div>
