  ...
  <div v-if="show" class="modal-overlay" @click.self="!isSubmitting && $emit('close')">
    <div class="modal-content" role="dialog" aria-modal="true" aria-labelledby="booking-modal-title">
      <div class="modal-header">
        ...
        <button class="close-btn" @click="$emit('close')" :disabled="isSubmitting" :aria-label="isSubmitting ? 'Действие недоступно во время загрузки' : 'Закрыть (Esc)'" :title="isSubmitting ? 'Действие недоступно во время загрузки' : 'Закрыть (Esc)'">×</button>
      </div>
