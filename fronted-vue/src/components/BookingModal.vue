  ...
  <div v-if="show" class="modal-overlay" @click.self="!isSubmitting && $emit('close')">
    <div class="modal-content" role="dialog" aria-modal="true" aria-labelledby="booking-modal-title">
      <div class="modal-header">
        ...
        <span style="display: inline-flex;" :title="isSubmitting ? 'Закрытие недоступно во время сохранения' : 'Закрыть (Esc)'">
          <button class="close-btn" @click="$emit('close')" :disabled="isSubmitting" :aria-label="isSubmitting ? 'Закрыть (Esc) - Действие недоступно во время сохранения' : 'Закрыть (Esc)'">×</button>
        </span>
      </div>
