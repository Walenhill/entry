<template>
  <div class="card slot-card" :class="`status-${statusClass}`">
    ...
    <div class="slot-footer">
      ...
      <button @click="$emit('delete', slot.id)" class="btn btn-outline btn-icon" :title="(isCanceling || isDeleting) ? 'Действие недоступно во время загрузки' : 'Удалить слот'" :aria-label="`Удалить слот на ${slot.date} с ${slot.start_time} до ${slot.end_time}`" :disabled="isDeleting || isCanceling">
