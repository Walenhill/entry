  ...
  <div class="card slot-card" :class="`status-${slot.statusClass}`">
    ...
    <div class="slot-body">
      ...
      <dl v-if="slot.is_booked" class="booking-details">
        ...
        <div class="detail-item" v-if="slot.booking_comment">
          ...
          <dd class="value">
            <a :href="`tel:${slot.booking_comment}`" class="phone-link" :aria-label="`Позвонить клиенту: ${slot.booking_comment}`" title="Нажмите, чтобы позвонить" style="display: inline-flex; align-items: center; gap: 0.25rem;">
              <span aria-hidden="true" style="font-size: 0.9em;">📞</span>
              {{ slot.booking_comment }}
            </a>
          </dd>
        </div>
      </dl>
    ...
    <div class="slot-footer">
      <template v-if="!slot.is_booked">
        <span :title="(isCanceling || isDeleting) ? 'Забронировать слот - Действие недоступно во время загрузки' : 'Забронировать слот'" style="display: inline-flex; flex: 1;">
      ...
      <template v-else>
        <span :title="(isCanceling || isDeleting) ? 'Отменить бронь - Действие недоступно во время загрузки' : 'Отменить бронь'" style="display: inline-flex; flex: 1;">
