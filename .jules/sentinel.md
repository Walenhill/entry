...
## 2026-07-10 - Prevent Information Disclosure via Browser Cache
**Vulnerability:** Missing Cache-Control and Pragma headers on API responses.
**Learning:** By default, browsers and intermediate proxies may cache sensitive JSON responses (like admin slots or statistics), exposing them to unauthorized users who gain access to the browser history or shared proxy cache.
**Prevention:** Explicitly set strict anti-caching headers (`Cache-Control: no-store, no-cache, must-revalidate, max-age=0` and `Pragma: no-cache`) in the main API routing file to enforce that all sensitive dynamic responses are never cached.
