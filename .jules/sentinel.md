...
## 2024-10-27 - Missing Rate Limiting on Unauthenticated Public API Endpoints
**Vulnerability:** The public POST /slots/{id}/book endpoint lacked rate limiting, allowing an attacker or automated script to aggressively book all available slots, leading to resource exhaustion and business denial of service.
**Learning:** Public, unauthenticated endpoints that change application state are prime targets for automated abuse. Relying only on rate limiting for authentication endpoints is insufficient.
**Prevention:** Always implement IP-based rate limiting on sensitive, unauthenticated endpoints to prevent automated abuse and resource exhaustion, rather than relying on session-based rate limiting which can be bypassed by omitting cookies.
