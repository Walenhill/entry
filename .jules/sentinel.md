...
## 2024-08-04 - [CSRF Bypass via Loose Content-Type Parsing]
**Vulnerability:** The API used `stripos($contentType, 'application/json') !== false` to enforce JSON payloads for CSRF protection. An attacker could bypass this by sending a simple POST request (e.g. from an HTML form) with `Content-Type: text/plain; application/json`, which passes the `stripos` check but doesn't trigger a CORS preflight.
**Learning:** Simple substring matching for `Content-Type` is insufficient for CSRF protection because attackers can inject the required string into the parameters section of a harmless media type.
**Prevention:** Always parse the `Content-Type` header properly (e.g., splitting by `;` and trimming) and use strict string comparison (`strcasecmp`) against the primary media type (e.g., `application/json`) to ensure the request genuinely requires a preflight.
