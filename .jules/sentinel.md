...
## 2026-07-15 - Information Disclosure via Browser Cache
**Vulnerability:** The API lacked proper cache control headers (`Cache-Control`, `Pragma`). Sensitive JSON API responses containing bookings and metadata could be cached by the browser and potentially accessed locally.
**Learning:** Browsers and intermediate proxies can aggressive cache responses by default, which can lead to information disclosure if another user accesses the same machine or if local caches are examined.
**Prevention:** Always explicitly set strict anti-caching headers (`Cache-Control: no-store, no-cache, must-revalidate, max-age=0` and `Pragma: no-cache`) for all sensitive or dynamic API responses.
