...
## 2026-09-02 - [Strict Content-Type Validation to Prevent CSRF Bypass]
**Vulnerability:** The application used a loose substring match (`stripos($contentType, 'application/json')`) to validate the Content-Type header on state-changing endpoints, allowing attackers to bypass CSRF protections by appending `; application/json` to a simple request like `text/plain`.
**Learning:** Using loose substring matching for security-critical headers can be bypassed by attackers crafting payloads that satisfy the match while remaining valid for a different intended behavior.
**Prevention:** Always use strict parsing (e.g., splitting by `;` and comparing the primary media type with `strcasecmp`) when validating the Content-Type header to enforce CSRF protections.
