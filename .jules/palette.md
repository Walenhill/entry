...
## 2026-05-17 - Disabled Form Controls and Tooltips
**Learning:** Found that disabled form controls (like `<button disabled>`) are inert and drop `title` attributes. This removes necessary context (like explanations for why an action is blocked) for both mouse users (who lose hover tooltips) and screen reader users (who lose the accessible description).
**Action:** When providing an explanation for a disabled action button (e.g., during asynchronous loading), wrap the disabled button in a `<span>` with `style="display: inline-flex;"` to hold the `title` attribute, and append the blocked state reason directly to the original `aria-label` (e.g., `aria-label="[Action] - Действие недоступно во время загрузки"`).
