...
## 2026-07-03 - Disabled States for Custom Inputs
**Learning:** Custom toggle buttons (like password visibility toggles) and complex form inputs in Vue do not automatically receive or communicate their disabled state correctly, both visually and semantically, leading to potential user confusion and accessibility gaps during loading states.
**Action:** When binding the loading state (e.g. `isLoading` or `isSubmitting`) to `:disabled` on form components, ensure explicit `:disabled` CSS rules (like `opacity: 0.6; cursor: not-allowed;`) are added. If using custom nested tooltips or `aria-labels`, dynamically append the "loading" context (e.g. `'Действие недоступно во время загрузки'`) to the `aria-label` and wrap the disabled button in a `<span>` to retain its tooltip.
