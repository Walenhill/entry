...
## 2026-05-17 - Disabled Elements and Tooltips Accessibility
**Learning:** Found that standard HTML `button` elements do not show tooltips (from the `title` attribute) or convey proper contextual state to screen readers when they are `disabled`. This is because disabled form controls are essentially inert and block pointer events, preventing the hover state needed for standard tooltips. Replacing the `aria-label` completely also removes context for the user.
**Action:** When explaining disabled action buttons (like icon-only buttons or modal close buttons) during asynchronous operations, wrap the disabled button in a `<span>` to hold the `title` attribute. Additionally, append the blocked state reason to the original `aria-label` dynamically (e.g., `aria-label="[Action] - Действие недоступно во время загрузки"`) to preserve context while explaining the block to screen readers.
