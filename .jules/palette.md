...
## 2024-07-02 - Preserve tooltips on disabled buttons
**Learning:** Disabled form controls (like `<button>`) become completely inert, preventing mouse events from firing. This causes standard `title` attributes or custom tooltip components placed directly on the disabled element to drop, leaving sighted users without context on *why* the action is unavailable. Additionally, screen readers often need explicitly concatenated status updates when a previously active button becomes disabled.
**Action:** When disabling interactive action buttons (like submit or cancel) during asynchronous operations, wrap the disabled element in a `<span>` or `<div>`. Apply the `title` attribute to this wrapper to retain hover context, ensure the layout is maintained (e.g., using `display: flex` and `flex-1`), and dynamically append the blocked status to the underlying `<button>`'s `aria-label` (e.g., `" - Действие недоступно во время загрузки"`) to guarantee screen reader announcements.
