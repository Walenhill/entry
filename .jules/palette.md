...
## 2026-08-25 - Focus Rings for Structural Buttons
**Learning:** Standalone structural buttons (like `.close-btn`, `.menu-toggle`, or `.close-mobile`) lack clear focus rings by default, hindering keyboard navigation accessibility.
**Action:** Always explicitly define `:focus-visible` states using `outline: none` and a themed `box-shadow` to ensure keyboard accessibility and compliance with accessibility checks.
