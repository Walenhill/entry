...
## 2026-07-23 - Prevent Whitespace-Only Submissions in Vue Forms
**Learning:** Found that required form inputs in Vue allow whitespace-only strings to pass validation, leading to empty or invalid data submissions (e.g., booking names with just spaces).
**Action:** Consistently apply the `v-model.trim` modifier on text inputs alongside the HTML5 `pattern=".*\S+.*"` attribute and a descriptive `title` attribute for native browser validation.
