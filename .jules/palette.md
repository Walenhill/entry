...
## 2026-05-18 - Skip to Content Links and Focus Management
**Learning:** Found that keyboard users lack a way to bypass repetitive navigation blocks (like sidebars). When programmatic focus shifts (like a 'Skip to Content' link) target a container element (e.g., <main>), the target element must be explicitly made focusable but hidden from tab sequences.
**Action:** Always implement a 'Skip to Content' link at the top of the DOM using absolute positioning that becomes visible on focus. Additionally, always add tabindex="-1" and style="outline: none;" to the target container element (e.g., <main>) to ensure cross-browser focus shift functionality without displaying an unsightly focus ring.
