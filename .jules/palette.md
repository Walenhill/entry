...
## 2026-05-17 - Dynamic Title Attributes on Icon-Only Buttons During Async Operations
**Learning:** Icon-only action buttons in the Vue frontend that get disabled during asynchronous operations (like deletions) create ambiguity if their static `title` attribute remains unchanged (e.g., showing "Удалить слот" while disabled). This fails to explain to users *why* the button is unavailable.
**Action:** When disabling icon-only action buttons during asynchronous operations, dynamically update their `title` attribute to explain the blocked state (e.g., `Действие недоступно во время загрузки`) instead of displaying the static action name.
