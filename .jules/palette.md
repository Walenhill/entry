...
## 2026-07-10 - Disable Form Inputs and Preserve Tooltips on Icon Buttons
**Learning:** When a form is submitting, binding the loading state exclusively to the submit button leaves other form inputs and custom icon buttons (like password toggles) interactable, causing potential duplicate state or confusion. Furthermore, setting 'disabled' on a <button> causes browsers to drop pointer events and tooltips ('title').
**Action:** Always bind the loading state (e.g., 'isLoading') to the ':disabled' attribute of *all* inputs in a form. To preserve tooltips on disabled custom icon buttons, wrap the button in a <span> to carry the 'title', use inline styles if needed for positioning, and ensure a ':disabled' CSS pseudo-class is present to communicate the inert visual state.
