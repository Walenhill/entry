...
## 2024-07-10 - Form Input Disabled States
**Learning:** Adding `:disabled="isSubmitting"` (or `isLoading`) to form inputs (like `input` and `textarea`) alongside the submit button significantly improves frontend UX by preventing users from modifying data mid-submission or triggering duplicate requests. When applying `disabled` to custom interactive elements (like an SVG password toggle icon wrapped in a `<button>`), it requires an explicit `:disabled` CSS rule (`opacity: 0.5; cursor: not-allowed;`) because it does not inherit default browser disabled styles like standard `<input>` elements do. Also, wrapping disabled buttons in a `<span>` to hold a `title` tooltip is crucial because `disabled` elements swallow mouse events.
**Action:** Always ensure all inputs within a form are bound to the same loading state as the submit button. Apply explicit CSS disabled states to custom button components, and use `<span>` wrappers for their tooltips.
