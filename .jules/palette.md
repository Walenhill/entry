...
## 2026-05-18 - Body Scroll Locking for Overlays
**Learning:** Found that modals and mobile sidebars do not prevent background scrolling. This can cause users to accidentally scroll the main page context away while interacting with the overlay, which is especially jarring on touch devices.
**Action:** Always add `document.body.style.overflow = 'hidden'` when opening full-screen overlays (like modals or mobile sidebars) and restore it to `''` when they close or unmount to preserve the scroll position of the underlying content.
