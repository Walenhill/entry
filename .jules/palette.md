...
## 2026-05-18 - Logout Button Loading State & Menu Title Tooltips\n**Learning:** Found that the logout button lacked a loading state, allowing potential multiple clicks during network latency. Also, mobile menu toggle/close buttons used abstract icons (☰ and ×) without `title` tooltips, leaving mouse users without hover context.\n**Action:** Always add an `isLoading` state to asynchronous action buttons (disabling them during the process). Add `title` attributes to icon-only buttons to supplement `aria-label`s for sighted users.
