...
## 2026-08-03 - Prevent background scrolling on full-screen overlays
**Learning:** Found that opening full-screen overlays (like modals and mobile sidebars) does not prevent the background from scrolling, which can confuse users, especially on mobile devices.
**Action:** Set `document.body.style.overflow = 'hidden'` when the overlay opens and explicitly restore it to `''` when it closes or unmounts.
