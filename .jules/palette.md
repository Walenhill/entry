...
## 2026-06-19 - Accessible Disabled Action Buttons Tooltips
**Learning:** Found that applying the `title` attribute directly to a disabled native `<button>` element does not produce a tooltip in most major browsers because disabled elements are inert and do not receive pointer events. Additionally, replacing the `aria-label` with only the disabled explanation (e.g. "Action unavailable during loading") destroys the accessible name, confusing screen reader users about what the button actually does.
**Action:** When explaining disabled action buttons (like icon-only buttons or modal close buttons) during asynchronous operations, wrap the disabled button in a `<span>` or `<div>` to hold the `title` attribute and ensure it has `display: flex; cursor: not-allowed;` so the tooltip displays on hover. Instead of replacing the original `aria-label`, prepend or append the disabled explanation (e.g., `aria-label="Действие недоступно во время загрузки. [Original Action Name]"`) to preserve semantic context while communicating the disabled state.
