...
## 2024-08-30 - Missing Focus Rings on Structural Buttons
**Learning:** Structural icon-only buttons (like .close-btn, .menu-toggle, and .close-mobile) in this app's components do not inherit the global .btn focus styles, resulting in no clear visual indicator during keyboard navigation.
**Action:** Always explicitly define :focus-visible states using outline: none and a themed box-shadow (e.g., 0 0 0 2px var(--bg-surface), 0 0 0 4px var(--accent-secondary)) for custom structural buttons to ensure keyboard accessibility.
