...
## 2026-07-06 - Persistent Database Connections in mysqli
**Learning:** Initializing a new database connection for every API request introduces significant TCP handshake and authentication overhead. PHP's mysqli extension natively supports connection pooling via persistent connections.
**Action:** Prefix the database host string with `p:` (e.g., `p:$host`) when instantiating the `mysqli` object to reuse connections across requests, drastically reducing connection latency.
