...
## 2024-05-18 - Prevent O(N) re-renders in unpaginated lists with parent reactive state
**Learning:** In the Vue application, the list of slots (`slotsStore.slots`) is rendered as an unpaginated list. When the parent component (`SlotsView.vue`) handles asynchronous actions (like canceling or deleting a slot), it stores the active ID in local reactive state (`cancelingSlotId`, `deletingSlotId`). Because these state variables change, Vue's reactivity system triggers a re-render of the entire `SlotsView` component, evaluating and diffing VNodes for *all* `SlotCard` components in the `v-for` loop, leading to O(N) complexity for a single slot update.
**Action:** Always apply the `v-memo` directive to components within a `v-for` loop when rendering large lists where parent state triggers updates. Set the dependency array to include the item data and the specific boolean conditions related to that item (e.g., `v-memo="[item, activeItemId === item.id]"`). This skips VNode creation for all unchanged items, turning an O(N) update into an O(1) update.
