...
## 2026-07-05 - Optimize database connections
**Learning:** Establishing new TCP connections and authenticating with the MySQL database on every single API request adds significant latency overhead, especially in high-traffic APIs where many concurrent requests hit the database.
**Action:** Use persistent connections (`p:` prefix for the host in mysqli) to simulate connection pooling natively in PHP, which heavily reduces connection times for subsequent requests.
