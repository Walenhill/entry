...
## 2026-06-29 - Using PHP Persistent Connections for MySQL Connection Pooling
**Learning:** Establishing a new database connection for every incoming HTTP request introduces significant latency due to the overhead of the TCP handshake and MySQL authentication.
**Action:** When using PHP's `mysqli` extension, you can simulate connection pooling by simply prefixing the database host with `p:` (e.g., `p:localhost`). This reuses established database connections across multiple requests, drastically reducing connection latency and improving overall application throughput.
