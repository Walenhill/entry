...
## 2024-11-20 - Preventing Session Lock Contention
**Learning:** PHP's default session handler locks the session file until the script terminates. This blocks concurrent requests from the same user (e.g., fetching slots and stats simultaneously, or during long-running operations), causing significant latency and waterfall loading in the frontend.
**Action:** Call `session_write_close()` early in read-only endpoints (e.g., GET requests) or long-running processes after authentication is complete to release the lock and allow parallel execution.
