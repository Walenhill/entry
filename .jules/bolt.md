...
## 2026-07-03 - Simulate database connection pooling
**Learning:** In the PHP backend, opening a new database connection for every single API request introduces significant TCP handshake and authentication latency.
**Action:** Simulate connection pooling to reduce TCP and auth overhead by prefixing the database host with `p:` (e.g., `p:localhost`) when establishing a new mysqli connection to use persistent connections.
