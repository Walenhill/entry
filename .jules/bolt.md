...
## 2026-06-21 - Connection Pooling via Persistent Connections
**Learning:** In the PHP backend, each API request normally incurs the overhead of establishing a new TCP connection and authenticating with the MySQL server via . This latency adds up, especially under load.
**Action:** Prefixing the database host string with `p:` (e.g., `p:localhost`) enables persistent connections in PHP's `mysqli` extension. This allows the process to reuse existing database connections, significantly reducing the overhead of TCP handshakes and authentication for subsequent requests.
## 2026-06-21 - Connection Pooling via Persistent Connections
**Learning:** In the PHP backend, each API request normally incurs the overhead of establishing a new TCP connection and authenticating with the MySQL server via new mysqli(). This latency adds up, especially under load.
**Action:** Prefixing the database host string with p: (e.g., p:localhost) enables persistent connections in PHP's mysqli extension. This allows the process to reuse existing database connections, significantly reducing the overhead of TCP handshakes and authentication for subsequent requests.
