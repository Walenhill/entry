...
## 2024-06-13 - Remove redundant SELECT before DELETE
**Learning:** In PHP backend CRUD operations, an anti-pattern exists where a redundant `SELECT` query is executed solely to verify a record's existence before a `DELETE` or `UPDATE` mutation.
**Action:** Avoid this read-before-write anti-pattern by executing the mutation directly and checking `$stmt->affected_rows` to determine if the record existed and was successfully modified, eliminating an unnecessary database roundtrip.
...
## 2024-06-27 - Preventing O(N) Reactive Effect Overhead in v-for
**Learning:** In Vue 3, child components rendered within a `v-for` loop that receive completely new object properties on update (rather than having their existing properties deeply mutated) suffer unnecessary initialization overhead if they use `computed` properties. `computed` properties instantiate reactive effects, which consumes memory and CPU. For very simple derivations based on props (like checking a boolean `is_booked`), this overhead scales linearly (O(N)) with the list size.
**Action:** When working with large lists of child components that receive fresh object props from the parent store, replace trivial `computed` properties with inline template expressions (like ternary operators) to eliminate the overhead of generating redundant reactive effects.
