...
## 2026-06-08 - Eliminating Redundant Database Reads Pre-Mutation
**Learning:** In PHP backend CRUD operations, a redundant `SELECT` query is often executed solely to verify a record's existence before a `DELETE` or `UPDATE` mutation. This introduces an unnecessary database roundtrip.
**Action:** Execute the mutation directly and check `$stmt->affected_rows` to determine if the record existed and was successfully modified, eliminating an unnecessary database roundtrip.
