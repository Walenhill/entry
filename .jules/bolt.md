...
## 2026-06-20 - Connection Pooling with Persistent Connections
**Learning:** Instantiating a `new mysqli()` connection on every backend API request incurs significant overhead due to repetitive TCP handshakes and database authentication. In high-traffic scenarios or endpoints that execute many queries sequentially without grouping (though that is an anti-pattern itself), this connection latency accumulates and impacts overall response times.
**Action:** In PHP, simulate database connection pooling to reduce TCP handshake and authentication latency by prefixing the database host string with `p:` (e.g., `p:localhost`) when instantiating `mysqli`. This utilizes persistent connections at the `mysqlnd` layer, dramatically improving connection speed for subsequent requests.
