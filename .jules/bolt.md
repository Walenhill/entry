...
## 2024-06-27 - Persistent MySQLi Connections for API Performance
**Learning:** Instantiating `new mysqli($host, ...)` on every API request incurs full TCP handshake and authentication latency with the database server, which slows down high-frequency endpoints.
**Action:** Utilize PHP's built-in connection pooling for MySQLi by prefixing the database host with `p:` (e.g., `p:localhost`). This reuses open connections across requests, drastically reducing database connection overhead without requiring architectural changes.
