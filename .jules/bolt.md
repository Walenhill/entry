...
## 2026-06-15 - Eliminating O(N) Reactive Effects in Vue Child Components
**Learning:** In Vue frontend development, defining `computed()` properties inside child components (like `<SlotCard>`) that are rendered via a `v-for` loop creates significant performance overhead. If the item data is immutable and completely replaced by the parent upon update, these redundant O(N) reactive effects consume excessive memory and initialization time for large lists.
**Action:** Avoid using `computed()` inside such child components. Instead, use inline template expressions (e.g., ternary operators) or resolve the computed logic earlier in the data pipeline to prevent the creation of unnecessary reactivity watchers.
