...
## 2024-06-27 - Preventing O(N) Reactive Effects in v-for Loops
**Learning:** In Vue, when rendering child components inside a `v-for` loop, defining `computed()` properties within the child component (e.g. `SlotCard.vue`) forces Vue to set up reactive dependencies for every single item in the list. If the underlying data is largely immutable or completely replaced by the parent on update, this deep reactivity setup is completely redundant and causes O(N) memory allocation and initialization overhead.
**Action:** Replace `computed()` properties in list child components with simple inline ternary expressions in the template when the data structure does not require deep, granular reactivity tracking. This eliminates the reactive setup cost for large unpaginated lists.
