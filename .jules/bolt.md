...
## 2024-08-03 - Prevent Session Lock Contention
**Learning:** In PHP, active sessions lock the session file until the request completes, blocking concurrent requests from the same user. This creates a severe performance bottleneck for parallel API calls or long-running requests when they only need to verify authentication once.
**Action:** Explicitly call `session_write_close()` early in read-only endpoints or long-running processes after authentication is complete to release the lock.
