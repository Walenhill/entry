...
## 2026-07-07 - Connection pooling
**Learning:** PHP scripts reconnect to the database on every HTTP request which introduces overhead. Connection pooling reduces latency.
**Action:** Prefix the host string with `p:` in mysqli to enable persistent connections, effectively simulating connection pooling.
