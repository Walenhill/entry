...
## 2024-06-27 - Persistent Database Connections in PHP
**Learning:** In the PHP backend API, establishing a new database connection for every single HTTP request incurs significant latency due to the TCP handshake and authentication overhead, particularly under high load.
**Action:** In `mysqli`, prefix the database host string with `p:` (e.g., `p:localhost`). This instructs PHP to utilize persistent connections, effectively simulating connection pooling and reusing existing connections across requests, drastically reducing database connection latency.
