...
## 2026-06-21 - Enabling Persistent Connections in PHP (mysqli)
**Learning:** In PHP, `mysqli` opens a new TCP connection on every request. This is a massive performance bottleneck under high load due to the latency of the TCP handshake and MySQL authentication.
**Action:** Enable persistent connections (connection pooling via `mysqlnd`) by prefixing the `DB_HOST` with `p:` (e.g., `p:localhost`). This allows PHP processes to reuse open connections, drastically reducing request latency and database CPU usage.
