...
## 2026-07-11 - Enable PHP Persistent Connections (Connection Pooling)
**Learning:** Establishing a new database connection for every API request incurs significant overhead for TCP handshaking and MySQL authentication, which can bottleneck performance under high load.
**Action:** When using `mysqli` in PHP, simulate connection pooling by prefixing the database host string with `p:` (e.g., `p:localhost`). This instructs PHP to reuse existing idle connections, drastically reducing latency for subsequent database operations.
