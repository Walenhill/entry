...
## 2026-08-28 - Caching CORS Preflight Requests
**Learning:** For APIs involving credentials, browsers send a preflight OPTIONS request before actual requests. Without caching, this adds a redundant network round-trip for every API call, increasing latency.
**Action:** Set the Access-Control-Max-Age header (e.g., 86400) alongside other CORS headers in the main entry point to cache the preflight response in the browser, eliminating redundant network requests.
