...
## 2025-05-28 - [Caps Lock Warning]
**Learning:** Users often fail logins because Caps Lock is on, leading to frustration and potential account lockouts. A real-time, screen-reader accessible warning (using `role="alert"` and `aria-live="polite"`) provides immediate, actionable feedback before they submit the form.
**Action:** When implementing password inputs, always include a real-time Caps Lock check via keyboard/mouse events and display an accessible warning if active.
