...
## 2024-05-28 - [Structural Buttons Focus Visibility]
**Learning:** Standalone structural buttons (like `.close-btn`, `.menu-toggle`, or `.close-mobile`) often lack `.btn` utility classes and consequently lose global focus styles. Without explicit `:focus-visible` definitions, they become invisible to keyboard users navigating the application, breaking accessibility.
**Action:** Always explicitly define `:focus-visible` states with `outline: none` and a themed `box-shadow` for structural buttons to ensure keyboard accessibility and compliance.
