...
## 2025-05-28 - [Structural Button Focus Indicators]
**Learning:** Standalone structural buttons (like .close-btn, .menu-toggle, or .close-mobile) lack clear focus rings by default. Without explicit :focus-visible styles, they fail keyboard accessibility checks because users navigating with a keyboard have no visual feedback that the element is focused.
**Action:** Always explicitly define :focus-visible states using outline: none and a themed box-shadow on all standalone or custom buttons to ensure keyboard accessibility.
